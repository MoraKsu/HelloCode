...
{   
  //return n == 0 ? 1 : PowerRec(a, n - 1) * a; // краткая запись
  if (n == 0) return 1;
...
void FindWords(string alchabet, char[] word, int length = 0)
{
  if (length == word.Length)
  {
    Console.WriteLine($"{n++} {new String(word)}"); return;
  }
  for (int i = 0; i < alchabet.Length; i++)
  {
    word[length] = alchabet[i];
    FindWords(alchabet, word, length + 1);
  }
}
...
string path = @"C:\Users\MoRa\Desktop\Examples";
CatalogInfo(path);
*/
